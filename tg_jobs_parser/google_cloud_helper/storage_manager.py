...
class StorageManager:
    ...
    def list_msgs_with_metadata(self):
        try:
            bucket = storage.Bucket(self.client, self.config.bucket_name, user_project=self.config.project)
            all_blobs = list(self.client.list_blobs(bucket, prefix=self.config.source_msg_blob))
            files_metadata = []
    ...
    def check_channel_stats(self):
        tmp_path = os.path.join(volume_folder_path, 'tmp_gsc_channels_metadata_for_stats.json')
    ...
    def log_statistics(self):
        ...
        logging.info(f'channels_to_update_ids: {self.statistics["to_upd_channels_ids"]}')
